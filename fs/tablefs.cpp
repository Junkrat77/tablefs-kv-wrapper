...
std::string TableFS::InitInodeValue(const std::string& old_value,
 ...
 /* 
  tfs_inode_val_t ival;
  ival.size = TFS_INODE_HEADER_SIZE + filename.size() + 1;
  if (old_header->has_blob == 0)
    ival.size += old_header->fstat.st_size;
  ival.value = new char[ival.size];
  *header = *old_header;
  header->namelen = filename.size();
  memcpy(ival.value + TFS_INODE_HEADER_SIZE , filename.data(), filename.size());
  strncpy(header->buffer, filename.data(), filename.size());

  std::string value = std::string(
  if (header->has_blob == 0 && header->fstat.st_size > 0) {
    memcpy(ival.value + TFS_INODE_HEADER_SIZE +(header->namelen)+1,
           old_header->buffer+(old_header->namelen)+2,
